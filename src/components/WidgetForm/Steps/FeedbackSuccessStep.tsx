...
export function FeedbackSuccessSetp({onHandleRestartFeedback}:Props) {
  return (
      <>
      <header>
          <CloseButton/>
      </header>
    <div className="flex items-center justify-center w-[375px] flex-col py-10">
      <svg
        width="41"
        height="40"
        viewBox="0 0 41 40"
      ...
      type="button" 
      className="bg-zinc-800 py-2 mt-6 px-6 rounded-md text-sm leading-6 border-transparent hover:bg-zinc-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-zinc-900 focus:ring-brand-500 transition-colors disabled:opacity-50 disabled:bg-brand-500 transaction-colors">Quero enviar outro</button>
    </div>
    </>
  );
}
